...
use crate::http::state::{AssertionKey, StoredAssertion};

/// File descriptor set for gRPC reflection (OpenFGA service).
...
/// File descriptor set for the gRPC health service.
/// This is exported by tonic-health and needed for reflection to expose the health service.
pub const HEALTH_FILE_DESCRIPTOR_SET: &[u8] = tonic_health::pb::FILE_DESCRIPTOR_SET;
...
{
    // Create the OpenFGA service, optionally sharing assertions with HTTP
    ...
    if config.reflection_enabled {
        ...
        // Register health service file descriptors if health check is enabled
        // This allows clients to discover the health service via reflection
        if config.health_check_enabled {
            reflection_builder =
                reflection_builder.register_encoded_file_descriptor_set(HEALTH_FILE_DESCRIPTOR_SET);
        }

        let reflection_service = reflection_builder.build()?;
...
mod tests {
    ...
    #[test]
    #[allow(clippy::const_is_empty)]
    fn test_health_file_descriptor_set_is_valid() {
        // The health file descriptor set should be non-empty
        assert!(!HEALTH_FILE_DESCRIPTOR_SET.is_empty());
        // It should be a valid protobuf file descriptor set for the health service
    }
