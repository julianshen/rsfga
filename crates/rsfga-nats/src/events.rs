...
pub struct DlqMessage {
    ...
    ///
    /// For `ParseError` and `MaxRetriesExceeded`, this is the NATS JetStream
    /// delivery count from the original message. For `EventPublishFailed`,
    /// this is 0 because the CommittedEvent was generated by the consumer
    /// (not delivered by NATS) -- the event publisher's internal retry count
    /// is captured in the `error` field instead.
